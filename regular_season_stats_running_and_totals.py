...
# have a minutes played in game float
new_cols['MIN_float'] = [0.0]*players_aves.shape[0]
...
def running_min(group, run_tot, ave, mins_float):
...
for name, group in players_years_gp:
    ...
    # do minutes
    end_tot, end_ave = running_min(group, new_cols['MIN_TOT'], new_cols['MIN_AVE'], new_cols['MIN_float'])
    new_row['MIN'] = [end_tot]
    new_row['MPG'] = [end_ave]
    # do games appeared
...
# save as csv files
players_aves.to_csv('regular_season_players_running.csv')
totals.to_csv('regular_season_players_ending.csv')
...
# save as csv files
totals.to_csv('regular_season_teams_ending.csv')
games_aves.to_csv('regular_season_teams_running.csv')
